...
int main(void)
{
  ...
  // if(!ret){
  //   usart_puts1("CoAP init error.\r\n");
  //   return 0;
  // }

  // ret = CoAPSetResponseCallback(&ap, responseHandler);

  // usart_puts1("CoAPStart...\r\n");
  // ret = CoAPStart(&ap);
  // if(!ret){
  //   usart_puts1("CoAP Start error.\r\n");
  // }
  // // send CoAP packet
  ...
  //   sprintf(text, "publish: %d\r\n", cnt);
  //   usart_puts1(text);
  //   uint16_t ret = MicrogearPublishInt(&mg, "/nbiot/rssi", cnt);
  //   if(ret == 0){
  //     usart_puts1("Microgear publish error\r\n");
  //     break;
  //   }
  //   /* LED at PB9 ON */
  //   GPIO_SetBits(GPIOB,GPIO_Pin_7);
