...
#define APPID    "ATS01aa0000YppE0D"
#define KEY      "I5hmc2yTB15J4Mq"
#define SECRET   "K6bW3bNskPyerGp16ahXbDUM2"
...
void GPIO_setup(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5 | GPIO_Pin_1 | GPIO_Pin_10;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_40MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_DOWN;
...
bool sendNB(int retry){
	...
	for(int j=0;j<3;j++){
		...
		// Microgear feed
		sprintf(buffer, "temp1:%d,humi1:%d,temp2:%d,humi2:%d,pH:%d,vbat:%d", (int)(rawTemperature1*100.0f), (int)(rawHumidity1*10000.0f/105.0f), (int)(rawTemperature2*100.0f), (int)(rawHumidity2*10000.0f/105.0f), (int)(rawPH*100.0f), (int)(v_in_adc));
		usart_puts1(">>WriteFeed");
		packetid = MicrogearWriteFeed(&mg, "ATS01aa00005eeE0D", buffer);
		if(packetid){
