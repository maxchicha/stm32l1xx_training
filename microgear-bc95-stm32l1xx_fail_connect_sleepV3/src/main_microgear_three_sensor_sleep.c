...
volatile float rawTemperature1 = 0;
volatile float rawHumidity1 = 0;
volatile float rawTemperature2 = 0;
volatile float rawHumidity2 = 0;
volatile float rawPH = 0;
...
{  
	...
	for(volatile int i=0;i<3;i++){
		...
		// RSSI testing
		int8_t rssi = NBUartGetSignalStrength(&nb);
	...
	float v_in_adc = (float)(adc_value)/4095.0f*3.3f*1.5f*100.0f/3.86f*3.92f;
	ADC_deinit();
	...
	for(int j=0;j<3;j++){
		...
		// Microgear feed
		sprintf(buffer, "temp1:%d,humi1:%d,temp2:%d,humi2:%d,pH:%d,vbat:%d", (int)rawTemperature1, (int)rawHumidity1, (int)rawTemperature2, (int)rawHumidity2, (int)rawPH, (int)(v_in_adc));
