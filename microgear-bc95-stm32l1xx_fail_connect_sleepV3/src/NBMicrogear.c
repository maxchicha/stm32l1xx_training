...
bool MicrogearInit(Microgear* mg, UDPConnection* udp, UDPConnection* dns_udp, char *host, char *appid, char *key, char *secret){
	if(mg == NULL){
        NBUartSendDEBUG(mg->UDP->NB, "Microgear: Microgear null\r\n");
		return false;
	}

	if(udp == NULL){
        NBUartSendDEBUG(mg->UDP->NB, "Microgear: UDP null\r\n");
		return false;
	}

	if(dns_udp == NULL){
        NBUartSendDEBUG(mg->UDP->NB, "Microgear: DNS_UDP null\r\n");
		return false;
	}
	...
	if(!ret){
        NBUartSendDEBUG(mg->UDP->NB, "Microgear: CoAP init fail\r\n");
		return false;
	}

	ret = CoAPStart(&mg->CoAP);
	if(!ret){
        NBUartSendDEBUG(mg->UDP->NB, "Microgear: CoAP start fail\r\n");
		return false;
	}
	return true;
}

bool MicrogearStop(Microgear* mg){
    return CoAPStop(&mg->CoAP);
}
...
uint16_t MicrogearWriteFeed(Microgear *mg, char *feedid, char *payload){
	char *p;
    ...
    strcpy(p, "data=");
    strcat(p, payload);
    return MicrogearSend(mg, mg->buffer, p);
}
...
void MicrogearLoop(Microgear *mg){
	CoAPLoop(&mg->CoAP);
}
